...
public interface EscalationMasterRepository extends JpaRepository<EscalationMaster,Integer> {
    ...
    @Query(value = "SELECT em.appraisal_master_id , am.employee_id , e.businessunit , e.department , u.name AS user_name , em.initiated_by , em.status FROM escalation_master em INNER JOIN appraisal_master am ON em.appraisal_master_id = am.Id  INNER JOIN  employee e ON am.employee_id = e.Id  INNER JOIN  users u ON u.Id = e.user_id  WHERE  am.isActive = 1", nativeQuery = true)
    List<Object[]> findAllDetails();

    @Query(value = "SELECT em.appraisal_master_id , am.employee_id , e.businessunit , e.department , u.name AS user_name , em.initiated_by , em.status FROM escalation_master em INNER JOIN  appraisal_master am ON em.appraisal_master_id = am.Id  INNER JOIN  employee e ON am.employee_id = e.Id  INNER JOIN  users u ON u.Id = e.user_id  WHERE  am.isActive = 1 and department=:department " , nativeQuery = true)
    List<Object[]> findAllDetailsByDepartment(String department);

    @Query(value = "SELECT em.appraisal_master_id , am.employee_id , e.businessunit , e.department , u.name AS user_name , em.initiated_by , em.status FROM escalation_master em INNER JOIN  appraisal_master am ON em.appraisal_master_id = am.Id  INNER JOIN  employee e ON am.employee_id = e.Id  INNER JOIN  users u ON u.Id = e.user_id  WHERE  am.isActive = 1 and businessunit=:businessunit" , nativeQuery = true)
    List<Object[]> findAllDetailsByDesignation(String businessunit);
    @Query(value = "SELECT em.appraisal_master_id , am.employee_id , e.businessunit , e.department , u.name AS user_name , em.initiated_by , em.status FROM escalation_master em INNER JOIN  appraisal_master am ON em.appraisal_master_id = am.Id  INNER JOIN  employee e ON am.employee_id = e.Id  INNER JOIN  users u ON u.Id = e.user_id  WHERE  am.isActive = 1 and em.status=:status ", nativeQuery = true)
